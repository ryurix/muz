...
function u8fopen($file, $enc='windows-1251') {
...
function u8sub($s, $first, $count = 0) {
	if ($count > 0) {
...
function u8pos($s, $needle, $offset = 0) {
	return mb_strpos($s, $needle, $offset, 'UTF-8');
}

function u8rpos($s, $needle, $offset = 0) {
	return mb_strrpos($s, $needle, $offset, 'UTF-8');
}

function u8cap($s) {
	$s = trim($s);
	return u8upper(u8sub($s, 0, 1)).u8lower(u8sub($s, 1));
}

function u8capitalize($s) {
	$ss = explode(' ', $s);
	foreach ($ss as $k=>$v) {
		$ss[$k] = u8cap($v);
	}
