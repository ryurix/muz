...
$select = 'SELECT * FROM catalog WHERE i="'.$shop.'"'.$hide;
// $row = db_fetch_row($select);
$row = \Flydom\Memcached::fetchRow($select);
$root = '/catalog';
...
if ($row) {
	...
	if (\User::is('catalog')) {
		\Action::before('/store/new?catalog='.$row['i'], '+ Товар');
		\Action::before('/catalog/new?catalog='.$row['i'], '+ Раздел');

		if ($row['i']) {
			\Action::before('/catalog/'.$row['url'].'/edit', 'Изменить раздел');
			\Action::before('/catalog/'.$row['url'].'/erase', 'Удалить раздел');
		}
	}
	...
	if ($action == 'edit') {
		...
		if ($plan['']['valid']) {
			if ($plan['send']['value'] == 1) {
				...
				\Flydom\Alert::warning('Раздел изменен');
				w('catalog-cache');
//				\Page::redirect($root.'/'.$row['i']);
				...
				unlink($row['icon']);
				\Flydom\Alert::warning('Раздел вместе со всем содержимым удален');
				w('catalog-cache');
				\Page::redirect($root);
			}
		}
...
if (trim($url, '/') != trim(str_replace(' ', '+', \Page::url()), '/')) {
...
if (count($param)) {
	...
	if ($sub = db_fetch($q)) {
		...
		if (\User::is('catalog')) {
			\Action::before('/subcat?up='.$row['i'].'&code='.$code, 'Изменить подраздел');
		}
	} else {
		if (\User::is('catalog')) {
			\Action::before('/subcat?up='.$row['i'].'&code='.$code, 'Создать подраздел');
		}
	}
}
//*/
...
if ($getlen) {
	...
	//$q = db_query('SELECT * FROM canon WHERE url32='.$url32);
	$rows = \Db::fetchAll('SELECT * FROM canon WHERE url32='.$url32);
	$dt = 0;
	//while ($i = db_fetch($q)) {
	foreach ($rows as $i) {
		if ($url == $i['url'] && ($dt == 0 || $dt > $i['dt'])) {
			$canon = $i;
			$here = $i;
		}
	}
...
$w = 0;
if ($code32) {
	// $q = db_query('SELECT * FROM canon WHERE code32='.$code32);
	$rows = \Db::fetchAll('SELECT * FROM canon WHERE code32='.$code32);
	$dt = 0;
	// while ($i = db_fetch($q)) {
	foreach ($rows as $i) {
		if ($code == $i['code'] && $i['url'] == $url && ($dt == 0 || $dt > $i['dt'])) {
			$canon = $i;
			$here = $i;
		}
	}
...
if ($gets || (count($param) && count($canon) && $canon['canon']>=$w)) {
...
\Page::name($name);
$config['title2'] = str_replace(array('%name%', '%search%'), array($name, $search), \Page::dict(cache_get('catalog-title'.$tag_search)));
$config['title2'].= $tag_page;

if ($row['tag0'] && !strlen($search)) {
/*
	if (strlen($search)) {
		$row['tag1'] = str_replace(array('%name%', '%search%'), array($name, $search), \Page::dict(cache_get('catalog-description'.$tag_search)));
...
//*/
	...
	$config['description'] = str_replace(array('%name%', '%search%'), array($name, $search), \Page::dict(cache_get('catalog-description'.$tag_search)));
	$config['description'].= $tag_page;
	$config['keywords'] = str_replace(array('%name%', '%search%'), array($name, $search), \Page::dict(cache_get('catalog-keywords')));
