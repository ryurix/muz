...
class Wiki extends Text
{
	function parse($values = null) {
		$v = &$this->data;

		if (isset($v['value'])) {
			$v['value'] = $this->wiki_clean($v['value']);
		} else {
			$v['value'] = isset($v['default']) ? $v['default'] : '';
		}
		$v['valid'] = isset($v['min']) && mb_strlen(trim(strip_tags($v['value']))) < $v['min'];
	...
	function build($class = '') {
		$v = &$this->data;
		$v['value'] = $this->wiki_unclean($v['value']);
	...
	*/
	...
	// Table
						...
						for ($i = 0; $i < count($cells); $i++) {
							$cells[$i] = $this->wiki_line($cells[$i]);
						}
