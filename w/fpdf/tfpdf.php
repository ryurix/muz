...
function __construct($orientation='P', $unit='mm', $size='A4')
{
	...
	elseif(is_dir(dirname(__FILE__).'/ttf'))
		$this->fontpath = dirname(__FILE__).'/ttf/';
	else
		$this->fontpath = '';
	// Core fonts
...
function AddFont($family, $style='', $file='', $uni=false)
{
	...
	if ($uni) {
		...
		$unifilename = $this->_getfontpath().strtolower(substr($file ,0,(strpos($file ,'.'))));
		$name = '';
		$originalsize = 0;
		...
		if (!isset($type) ||  !isset($name) || $originalsize != $ttfstat['size']) {
			$ttffile = $ttffilename;
			require_once($this->_getfontpath().'ttfonts.php');
			$ttf = new TTFontFile();
